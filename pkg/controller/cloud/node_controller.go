...
func (cnc *CloudNodeController) AddCloudNode(obj interface{}) {
	...
	err := clientretry.RetryOnConflict(UpdateNodeSpecBackoff, func() error {
		...
		if instanceType, err := getInstanceTypeByProviderIDOrName(instances, curNode); err != nil {
			...
			glog.V(2).Infof("Adding node label from cloud provider: %s=%s", v1.LabelInstanceType, instanceType)
		...
		if zones, ok := cnc.cloud.Zones(); ok {
			...
			if zone.FailureDomain != "" {
				glog.V(2).Infof("Adding node label from cloud provider: %s=%s", v1.LabelZoneFailureDomain, zone.FailureDomain)
				curNode.ObjectMeta.Labels[v1.LabelZoneFailureDomain] = zone.FailureDomain
			}
			if zone.Region != "" {
				glog.V(2).Infof("Adding node label from cloud provider: %s=%s", v1.LabelZoneRegion, zone.Region)
